...
* 无限循环结构
* 只执行一次就被删除的结构
    * 删除后，代码依然驻留在 RAM 中，但是 OS 不再管理这段代码
...
* `OSRdyTbl[OS_LOWEST_PRIO/8+1]`
    * 数组长度为 8，8 * 8bit = 64bit
    * 每一位分别代表一个任务是否就绪
* `OSRdyGrp` 是一个变量 (8-bit)
    * 每一位代表了一组 8 个任务中是否就绪
    * 该组中只要有一个任务就绪，这一 bit 就要被置位
* 优先级也是一个变量 (8-bit)
...
任务调度所花的时间是常数，与所建任务数无关

调度过程属于临界区，需要关中断，防止被中断打断
...
禁止任务调度，使任务保持对 CPU 的控制权

实现原理：

* 对全局变量 - 锁定嵌套计数器 `OSLockNesting` 进行操作
* 该变量记录了被锁定的次数，允许嵌套深度 255 层
* 所谓加锁和解锁就是对该变量 +1 或 -1
...
没有其它任务进入就绪态时，该任务转入运行态

任务优先级永远为最低 - `OS_LOWEST_PRIO`
...
然后调 `OSStartHighRdy()` 运行就绪的最高优先级任务
