...
需要在编译前定义消息队列中的最大消息数 `OS_MAX_QS`

需要一个与消息队列最大消息数相同大小的 __指针数组__

* 分别指向每一条消息

还需要一个队列控制块 `OS_Q`，并链接到 ECB 的 `OS_EventPtr` 域上
...
* 查询 ECB 对应的消息控制块的 `OSQEntries` 是否大于 0
    * 大于 0，递减该变量，返回消息指针
    * 等于 0，则返回空指针
...
将 ECB 和队列控制块中的变量复制到 `OS_Q_DATA` 数据结构中
